...
def get_counterexample(group_table, demographic, metric_col="recall@10"):
    """
    Counterexample = worst/best demographic pair causing max fairness gap.
    """
...
def cegar_alpha_repair_loop(
    ...
    At each iteration:
      1. evaluate fairness
      2. verify gap <= eps
      3. if violated, identify worst group
      4. boost items preferred by worst group
      5. increase alpha
      6. re-evaluate
    """
    ...
    for it in range(max_iters):
        ...
        # SMT counterexample identifies the group to repair
        target_group = counterexample["worst_group"]

        # Use items preferred by the disadvantaged group as repair candidates
    ...
    return {
        ...
        "best_result": best_result,
        "history": pd.DataFrame(history),
        "message": "Could not satisfy fairness constraint within limits.",
    }